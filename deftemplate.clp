...
; Regla para verificar acceso
(defrule acceso-denegado
    ?intento <- (intento-acceso (usuario ?nombre-usuario) (zona ?nombre-zona))
    ?usuario <- (usuario (nombre ?nombre-usuario) (nivel-acceso ?nivel-acceso))
    ?zona <- (zona (nombre ?nombre-zona) (nivel ?nivel-zona))
    (test (< ?nivel-acceso ?nivel-zona))
    =>
    (printout t "Acceso denegado para " ?nombre-usuario " a la zona " ?nombre-zona ". Nivel de acceso insuficiente." crlf)
)
...
(defrule acceso-permitido
    ...
    (test (>= ?nivel-acceso ?nivel-zona))
    =>
    (printout t "Acceso permitido para " ?nombre-usuario " a la zona " ?nombre-zona ". Nivel de acceso suficiente." crlf)
)
